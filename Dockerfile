...
# Instala as dependências necessárias para instalar o Miniconda
RUN apt-get install -y wget bzip2 ca-certificates curl libglib2.0-0 libxext6 libsm6 libxrender1

# Baixa o arquivo .sh do Miniconda
RUN wget https://repo.anaconda.com/miniconda/Miniconda3-latest-Linux-x86_64.sh

# Instala o Miniconda em /opt/conda
RUN /bin/bash Miniconda3-latest-Linux-x86_64.sh -b -p /opt/conda && \
    rm Miniconda3-latest-Linux-x86_64.sh

# Adiciona o diretório bin do Miniconda ao PATH
SHELL ["/bin/bash", "-c"]
ENV PATH="/opt/conda/bin:$PATH"

# Instala as dependências do Graph Tool e Pygame
